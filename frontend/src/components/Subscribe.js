...
const Slider = ReactSlick.default?.default || ReactSlick.default?.Slider || ReactSlick.default;
    ...
    const [canModify, setCanModify] = useState(true);
    const [consumerPhoto, setConsumerPhoto] = useState(null);

    // State for date logic
    const [showCalendar, setShowCalendar] = useState(false);
    ...
    // Function to check if it's past the cutoff time
    const isPastCutoffTime = () => {
        const now = new Date();
        const cutoffHour = 22; // 10 PM
        const cutoffMinute = 30; // 30 minutes
    ...
    // Function to get the minimum selectable date for the calendar
    const getMinDate = () => {
        const now = new Date();
        const tomorrow = new Date(now);
        tomorrow.setDate(now.getDate() + 1);
    ...
    // Function to get the initial default date for new subscriptions
    const getInitialSubscriptionDate = () => {
        const now = new Date();
        const cutoffHour = 22;
    ...
    // Callback for date change in calendar
    const handleDateChange = (date) => {
        setSelectedDate(date);
        setDateSelectionError("");
    };

    // Callback to confirm selected date
    const confirmSubscriptionDate = () => {
        if (selectedDate < getMinDate()) {
            setDateSelectionError("Please select a valid start date.");
            return;
    ...
    // Fetch consumer profile with photo
    const fetchConsumerProfile = useCallback(async () => {
        if (!consumer?.consumer_id || !token) return;
        
        try {
            const response = await fetch(
                `http://localhost:5000/api/consumer/${consumer.consumer_id}`,
                {
                    headers: {
            ...
            if (response.ok) {
                ...
                // Construct photo URL same as in ConsumerProfile.js
    ...
    useEffect(() => {
        // Initialize selectedDate on component mount
        setSelectedDate(getInitialSubscriptionDate());

        // Fetch consumer profile photo
        ...
        const timer = setInterval(checkModificationWindow, 1000);
        return () => clearInterval(timer);
    }, [checkModificationWindow, consumer?.consumer_id, fetchConsumerProfile]);
    ...
    return (
        <div className="subscribe-page">
            ...
            {/* Updated Welcome Banner with Profile Photo */}
            <div className="welcome-container">
                <div className="welcome-content">
                    <div className="welcome-avatar">
                        {consumerPhoto ? (
                            <img 
                                src={`${consumerPhoto}?${Date.now()}`}
                                alt="Profile" 
                                className="welcome-profile-photo"
                                onError={(e) => {
                                    e.target.onerror = null;
                                    e.target.style.display = 'none';
                                    // Show the fallback icon when image fails to load
                                    const fallback = e.target.parentNode.querySelector('.welcome-icon');
                                    if (fallback) fallback.style.display = 'flex';
                                }}
                            />
                        ) : null}
                        <div className={`welcome-icon ${consumerPhoto ? 'fallback' : ''}`}>
                            <FaUser />
                        </div>
                    </div>
                    <div className="welcome-text">
