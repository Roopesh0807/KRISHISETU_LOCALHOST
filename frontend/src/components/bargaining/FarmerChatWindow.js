...
const FarmerChatWindow = () => {
  ...
  const generatePriceSuggestions = useCallback((basePrice, bidPrice) => {
    ...
    const interval = diff / 10; // Generate 10 suggestions with even intervals
    
    for (let i = 1; i <= 10; i++) {
      const newPrice = numericBid + (interval * i);
      if (newPrice < numericBase) {
        suggestions.push({
          amount: newPrice - numericBid,
    ...
    // Always add the base price as a final suggestion
    suggestions.push({
      amount: numericBase - numericBid,
      price: numericBase.toFixed(2),
      label: `₹${numericBase.toFixed(2)} (Original Price)`
    });
  ...
  const initializeSocketConnection = useCallback(() => {
    ...
    socket.current.on('bargainMessage', (message) => {
      if (message?.message_content) {
        ...
        if (message.sender_role === 'consumer') {
          const priceMatch = message.message_content.match(/₹(\d+\.?\d*)/);
          const price = priceMatch ? parseFloat(priceMatch[1]) : currentPrice;
          setCurrentPrice(price);

          // Generate counter suggestions for farmer based on the new consumer bid
          const suggestions = generatePriceSuggestions(originalPrice, price);
          setPriceSuggestions(suggestions);

          setWaitingForConsumerResponse(false);
          setShowPriceSuggestions(false); // Hide suggestions, show main actions
        }
      }
    });
    ...
    socket.current.on('bargainStatusUpdate', (data) => {
      ...
      setWaitingForConsumerResponse(false);
      setFreezeUI(true); // Freeze UI after a final status is set
      setShowPriceSuggestions(false); // Hide suggestions after a final status is set
    });
  ...
  }, [bargainId, token, generatePriceSuggestions, selectedConsumer, originalPrice]);
    ...
    const initializeChat = async () => {
      try {
        ...
        if (data.product) {
          ...
          setMinPrice(data.product.minimum_price);
  ...
  const handlePriceSelection = async (price) => {
    if (!price || parseFloat(price) < parseFloat(currentPrice)) {
      setError("Counter offer must be higher than the current bid price.");
            ...
            {!showPriceSuggestions && (
              <div className="krishi-bargain-actions">
                <div className="krishi-action-buttons">
                  ...
                  <button
                    onClick={() => {
                      setPriceSuggestions(generatePriceSuggestions(originalPrice, currentPrice));
                      setShowPriceSuggestions(true);
                    }}
